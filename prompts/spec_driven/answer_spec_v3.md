...
The reader is a nuclear-regulation expert who wants the **concrete substance** behind the answer — the figure with its basis, the precise sub-paragraph, the validity envelope, the edition in force — not a summary-level restatement. Slot for that substance. **If a `# PERSONA` block precedes this prompt, it states *who* the reader is, what their search purpose is, and the answer's spine (which facets lead, which authority anchors it, what to develop deeply) — let that block drive the decomposition: order `required_slots` and pick `governing_normative_class` to match its spine, and read each facet through its lens.** Absent a `# PERSONA` block, default to the licensing-reviewer reading: the reader already knows *which* regulation governs and needs the requirement's exact wording/value and how the staff judged compliance — anchor on the binding requirement and the review finding.
