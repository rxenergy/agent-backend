...
3. **A reference is either a *scope* or a *lexical anchor* — never both.** An `explicit_reference` (e.g. `10 CFR 50.46`, `RG 1.157`, `RG 1.206`, `10 CFR Part 50`, `NUREG-0800`) names a document. You have two ways to point a query at it, and you must pick exactly one per reference:
   ...
   - **Subsection / paragraph references count as the document name too — strip them when filtered.** A reference carrying a clause subsection or paragraph (`10 CFR 50.46(b)`, `10 CFR 50.46(a)(1)(i)`, `GDC 35(a)`, `Appendix K`, `§ 52.79(a)(1)`) still *names the document you filtered to*. When the query is `filter`-scoped to that document (e.g. `collection` `10CFR` and/or `canonical_id` `10CFR-Part1-50`), drop the **whole** reference — number, parentheses, and all — from `query_text`; the filter already restricts to that regulation, and the parenthesized id is not what discriminates *within* it. Example: filtering to `10CFR` for `10 CFR 50.46(b)` → `query_text` is `acceptance criteria peak cladding temperature maximum oxidation`, not `10 CFR 50.46(b) acceptance criteria …`. (Keep the subsection id verbatim only when it is a `cross_reference` to a *different* document than the query's scope, per the next paragraph.)
   A reference that points *elsewhere* than the query's scope (a `cross_reference` slot pointing at another clause/table while the query is scoped to a different document) is always a lexical anchor — keep it verbatim. The deterministic backstop will strip a scope-reference's name if it slips into `query_text` (including subsection/paragraph forms), but choose correctly so your reasoning matches the query.
...
Example F — **subsection references with parentheses, scope vs cross-reference**: a query about the `10 CFR 50.46(b)` acceptance criteria. Two things to get right: (1) when filtered to `10CFR`, the *subsection* reference `10 CFR 50.46(b)` is the document name — strip it whole, parentheses and all (rule 3, "subsection references count as the document name too"); (2) a different subsection cited as a pointer to *another* place is kept verbatim. Here the `50_46_b` slot filters to `10CFR` (and canonical_id `10CFR-Part1-50`), so `10 CFR 50.46(b)` is dropped from its `query_text`; but the `rg_1157` slot is filtered to `RG`, so the `10 CFR 50.46(a)(1)(i)` it points back to is a cross-reference to a *different* collection and stays verbatim:
{"reasoning":"50_46_b 슬롯은 10CFR/filter + canonical 10CFR-Part1-50/filter 라 그 규정으로 모집단 확정 → 자기 문서명 '10 CFR 50.46(b)' 를 (괄호 포함) query_text 에서 제거하고 acceptance criteria 개념 토큰만(rule 3 subsection scope). rg_1157 슬롯은 RG/filter 라 그 안의 '10 CFR 50.46(a)(1)(i)' 는 *다른* collection 을 가리키는 cross-reference → verbatim 유지(앵커).","queries":[{"slot_name":"rg_1157_performance_criterion","query_text":"ECCS performance criteria 10 CFR 50.46(a)(1)(i)","collection":"RG","collection_mode":"filter","canonical_id":"RG-1.157","canonical_id_mode":"filter"},{"slot_name":"50_46_b_acceptance_criteria","query_text":"acceptance criteria peak cladding temperature maximum oxidation","collection":"10CFR","collection_mode":"filter","canonical_id":"10CFR-Part1-50","canonical_id_mode":"filter"}]}
