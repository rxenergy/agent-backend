...
돈다. `spec_driven_v2` 는 LLM 업무를 두 노드로 분할해, **슬롯 단위 검증(verify_slot)과 외부
참조 문서 선별(follow_up)을 모두 Node2 로 떼어낸다** — Node1 은 spec/쿼리/생성만 돈다. 검증
결과로 N4 컨텍스트를 정제한다.
...
| 노드 | LLM(pool id) | 담당 |
|------|--------------|------|
| Node1 | `gemma-4-26b`(요청 resolved = utility_llm) | N1 Define Spec/slot, N2 Query Formulation, N4 Generation |
| Node2 | `gemma-4-26b-sub`(`SECONDARY_LLM`) | **슬롯 단위 1차 검색 결과 검증**(`retrieval.verify_slot`) + **외부 참조 문서 선별**(enhanced `retrieval.follow_up`) |
...
**병렬성**: 각 슬롯이 독립 task 로 돌아 배리어가 없다. slot 1 이 Stage2(외부참조 선별)로
넘어갈 때 slot 2 는 Stage1(검증)을 돈다 — 두 단계 모두 Node2 라 같은 vLLM 의 continuous
batching 으로 묶인다(검증·외부참조 호출이 같은 max_num_seqs 를 공유). 동시 슬롯 수는
...
- **verify 미가용/미배선**(Node2 검증) → verify 도구 graceful skip 또는 슬롯 fallback
  (method="fallback", necessary=전량, 멀티홉 없음) → 단일노드(v1식 전량 보존)로 동작.
- **Node2 미가용/미배선**(검증·외부참조 선별) → verify·follow_up graceful skip → 2차 검색
  없음·전량 보존(Node1 1차 검색 결과만으로 N4). Node2 복구는 `make up-onprem-sub`.
- **SECONDARY_LLM 빈 값** → `default_llm` 폴백(단일노드 — 검증·외부참조가 default_llm 노드에서
  돈다). **pool 에 없는 값** → boot fail-fast.
- follow_up·verify 도구 모두 SECONDARY_LLM pool 엔트리가 `openai_compat`(내부망 vLLM)일
  때만 배선(guided_json 요구 — anthropic/fake 비호환).
...
- 배선: `config/profiles.py`(follow_up·verify→SECONDARY_LLM + v2 프롬프트 source),
...
- `node1_llm_id` — 생성/쿼리를 돈 LLM(Node1).
...
`tool.vllm_verify.retrieval.verify_slot`(Node2)·`retrieval.follow_up`(Node2)·
