...
돈다. `spec_driven_v2` 는 LLM 업무를 두 노드로 분할해, 슬롯 단위 검증을 Node2 로 떼어내고
검증 결과로 N4 컨텍스트를 정제한다.

## 노드 분할

| 노드 | LLM(pool id) | 담당 |
|------|--------------|------|
| Node1 | `gemma-4-26b`(요청 resolved = utility_llm) | N1 Define Spec/slot, N2 Query Formulation, **N3.5 외부 참조 문서 선별**(enhanced `retrieval.follow_up`), N4 Generation |
| Node2 | `gemma-4-26b-sub`(`SECONDARY_LLM`) | **슬롯 단위 1차 검색 결과 검증**(`retrieval.verify_slot`) |

노드는 **pool id 로 핀**한다(IP/엔드포인트는 `LLM_POOL` env 가 소유). Node2 는 검증 도구
생성 시점에 주입돼 요청 `model` 을 추종하지 않는다(재현성 — 검증 모델은 고정 정책 입력).
...
N0 Triage(Node1) → N1 Define Spec(Node1) → N2 Query Formulation(Node1)
  ...
  → [per-slot 파이프라인 — 배리어 없이 동시]
       Stage1 Node2 verify(retrieval.verify_slot)
         입력: 사용자 질문 + answer_spec + 슬롯 쿼리 + 1차 검색 결과
         출력: 필요 청크 식별자 + 멀티홉 필요 청크 식별자
       Stage2 Node1 외부참조 선별(retrieval.follow_up, necessity_only)
         입력: 사용자 질문 + answer_spec + 슬롯 쿼리 + 멀티홉 청크
         출력: 답변에 꼭 필요한 외부 참조 + 재검색 쿼리
       Stage3 Node1 2차 검색(retrieval.search, 참조 문서 내부)
  → N4 Generation(Node1)
```

**병렬성**: 각 슬롯이 독립 task 로 돌아 배리어가 없다. slot 1 이 Stage2(Node1)로 넘어갈 때
slot 2 는 Stage1(Node2 검증)을 돈다 — 두 노드가 별개 vLLM 이라 실제로 겹친다. 동시 슬롯 수는
...
**Node2 가 고른 필요 청크(necessary) ∪ 2차(멀티홉) 결과만** 쓴다. 토큰 예산 거버너
...
- **Node2 미가용/미배선** → verify 도구 graceful skip 또는 슬롯 fallback(method="fallback",
  necessary=전량, 멀티홉 없음) → 단일노드(v1식 전량 보존)로 동작. Node2 복구는
  `make up-onprem-sub`.
- **SECONDARY_LLM 빈 값** → `default_llm` 폴백(단일노드). **pool 에 없는 값** → boot fail-fast.
- verify 도구는 SECONDARY_LLM pool 엔트리가 `openai_compat`(내부망 vLLM)일 때만 배선
  (guided_json 요구 — anthropic/fake 비호환).
...
- Node2 검증: `adapters/tools/retrieval_verify_slot.py` + `adapters/slot_verifier_llm.py`
...
- 배선: `config/profiles.py`(SECONDARY_LLM resolve + verify 도구 + v2 프롬프트 source),
...
- `node1_llm_id` — 생성/쿼리/외부참조를 돈 LLM.
- `verify` — `{node2, num_slots, total_necessary, total_multihop, added_second_pass,
...
`tool.vllm_verify.retrieval.verify_slot`(Node2)·`retrieval.follow_up`·`retrieval.search`
(Node1) tool span 이 슬롯별로 nesting 된다(task 생성 시점 컨텍스트 캡처). 도구 이름으로
Node2 호출이 메트릭에서 구분된다.
