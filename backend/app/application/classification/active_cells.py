...
# 기획 doc §3 Top Priority 5셀 + 기획 doc §7 "비활성 셀" 분기.
#
# 모드:
#   "all"          — 16개 셀 모두 active. 품질 게이트는 verification이 잡는다.
#   "top_priority" — 기획 §3 Top Priority 5셀만 active. 그 외는
...
TOP_PRIORITY_CELLS: frozenset[tuple[str, str]] = frozenset(
    {
        ...
        ("O3", "D3"),  # "NRC는 NuScale에 왜 이 RAI를 제기?"
        ("O2", "D4"),  # "RG 1.157의 요건 원문은?"
    }
)

# Backward-compat alias — keeps older imports working. Always points to the
# union ("all"); per-request gating uses `is_active(..., mode=...)`.
