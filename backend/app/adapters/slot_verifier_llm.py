"""SlotVerifierPort 구현체 — :class:`~app.ports.llm.LLMPort`(Node1 vLLM)로 슬롯 1개의
1차 검색 결과를 검증한다(spec_driven_v2 Node1 = main).
...
Node1 vLLM 에 나가는 호출 총량을 전역으로 묶는다(`max_num_seqs` 초과 발사 → 큐 적체·타임아웃
캐스케이드 방지). RetrievalVerifySlotTool 의 도구 레벨 캡과 중복이나, 어댑터 경계의 안전판.
...
# snippet 길이로 충분하고, Node1 KV-cache·디코딩 비용을 줄인다(공유 vLLM 안전판). 슬롯
# 일괄이라 청크 수만큼 곱해지나(≤20×1200자) Node1 context 안에 든다.
_CHUNK_RENDER_CHARS = 1200

# 동시 슬롯 호출 수 상한 — 러너 fan-out 전체에 걸친 *전역* 캡(인스턴스 싱글톤이라 모든
# 슬롯이 self._sem 을 공유). 슬롯당 호출 1회라 동시 호출 = 동시 슬롯 수. Node1 vLLM 의
...
_FALLBACK_RATIONALE_PREFIX = "⚠ Node1 검증 호출 실패 — 이 슬롯 청크 전량 보존"


class SlotVerifierLlm:
    """SlotVerifierPort 구현체 — Node1 LLMPort 주입형(async, 슬롯당 단일 호출).
