...
@pytest.mark.asyncio
async def test_regulation_formal_is_o2_d4() -> None:
...
@pytest.mark.asyncio
async def test_rai_rationale_is_o3_d3() -> None:
    r = await RuleClassifier().classify("RAI #1234를 NRC가 왜 제기했지? 배경 알려줘")
...
async def test_unknown_falls_back_to_default_with_zero_confidence() -> None:
    ...
    assert r.scenario_depth in ("D1", "D2", "D3", "D4")
