...
def _deps(tmp: Path, *, llm, with_verify: bool, with_judge: bool = True,
          retriever=None, verify_tool=None, rescope_tool=None) -> AgentDeps:
    # with_verify = retrieval.verify_slot 도구 배선 여부. with_judge = LLM-as-judge
    # verifier source 배선 여부(기본 True — 도구가 있으면 판정도 한다). 둘을 분리해
    # "도구는 있으나 source 미배선 → verdict=skipped(결정론 폴백 금지)" 케이스를 표현.
