...
def _deps_two_node(tmp: Path, *, node1, node2, verify_source) -> AgentDeps:
    """Node1(생성/쿼리) + 검증 도구를 sub 노드 vLLM 엔드포인트(node2 인자)에 배선한 2-노드
    deps. 검증은 swap 후 sub 노드(Node2 = secondary_llm) 업무로, follow_up 과 같은 노드에서
    돈다. 통합 테스트에선 검증 도구가 실 vLLM 에서 도는지 별도 엔드포인트로 단독 확인한다."""
...
async def test_two_node_end_to_end_verify_pin(node1_llm: HttpLLM) -> None:
    # Node1(생성) + Node2 검증 도구(별도 vLLM 엔드포인트) 둘 다 실 vLLM. verify 가 슬롯별로 돌고
